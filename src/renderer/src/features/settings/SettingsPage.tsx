...
export const SettingsPage: React.FC = () => {
  ...
  if (isLoading || !settings) {
    return (
      <div className="p-10 text-secondary font-bold uppercase text-xs">
        Accessing Secure Config...
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 max-w-4xl pb-20">
      <div>
        <h2 className="text-3xl font-black text-primary tracking-tight font-syne italic uppercase">
          Configuration
        </h2>
        <p className="text-secondary text-sm font-medium mt-1">
          Behavioral parameters for ScreenSavage.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 p-4 bg-primary/5 rounded-2xl border border-primary/10">
        ...
        <p className="text-[10px] text-primary/70 font-medium">
          Click "Save Changes" to commit parameters to the secure store.
        </p>
      </div>
    </div>
  );
...
const SettingSection = ({ title, icon, children }: any) => (
  <div className="bg-canvas border border-border rounded-[40px] overflow-hidden shadow-sm hover:border-primary/20 transition-colors">
